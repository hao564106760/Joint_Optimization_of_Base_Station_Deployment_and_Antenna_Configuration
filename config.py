...
def get_config():
    ...
    parser.add_argument("--num_env_steps", type=int, default=3e4,
    ...
    parser.add_argument("--episode_length", type=int,
                        default=12, help="Max length for any episode")
    ...
    # TODO 是否调用之前的模型
    parser.add_argument("--use_pretrain_model", action='store_false',
                        default=False, help="Whether to use use_pretrain_model")
    # TODO 智能体数量
    parser.add_argument("--agent_num", type=int, default=49,
    ...
    # ppo parameters
    # TODO
    #通常训练单个智能体的时候，我们会将数据切分成很多个mini - batch，并且在一个epoch中将其多次训练来提高数据的利用效率，
    # 但是作者在实践中发现，可能是由于环境的非平稳态问题，如果数据被反复利用训练的话效果会不太好，
    # 因此建议对于简单的task用15个epoch，比较困难的任务用10个或者5个epoch，并且不要将数据切分成多个mini - batch。
    # 当然也不是绝对的，作者说到了对于SMAC中的一个环境，将数据切分成两个mini - batch的时候有提高性能，
    # 对此作者给出了解释说有帮助跳出局部最优，还引用了一篇参考文献。这一波说辞不是自相矛盾么。。。。
