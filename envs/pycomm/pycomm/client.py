...
def _coverage_optimize_request_packer(
    ...
    for request_bs in request_dat:
        action = pb_coverage.CoverageAction(
            ...
            bs_control_state=request_bs.get("bs_control_state", None)
        )
...
def visualize_response_packer(response):
    result = []
    for output in response:
        visualize = {}
        visualize['step'] = output.step

        base_stations = []
        for base_station in output.base_stations:
        ...
        strengths = []
        for strength in output.heatmap.strength:
            strengths.append(strength)
        base_station_ids = []
        for base_station_id in output.heatmap.base_station_id:
            base_station_ids.append(base_station_id)
        freq_range_ids = []
        for freq_range_id in output.heatmap.freq_range_id:
            freq_range_ids.append(freq_range_id)
        rates = []
        for rate in output.heatmap.rate:
            rates.append(rate)
        unsatisfied_ratios = []
        for unsatisfied_ratio in output.heatmap.unsatisfied_ratio:
            unsatisfied_ratios.append(unsatisfied_ratio)
        visualize['heatmap'] = {'num_rows': output.heatmap.num_rows, 'num_columns': output.heatmap.num_columns,'strength': strengths,
                                    'base_station_id': base_station_ids,'freq_range_id': freq_range_ids,'rate': rates,'unsatisfied_ratio': unsatisfied_ratios}
        
        strengths = []
        for strength in output.small_heatmap.strength:
            strengths.append(strength)
        base_station_ids = []
        for base_station_id in output.small_heatmap.base_station_id:
            base_station_ids.append(base_station_id)
        freq_range_ids = []
        for freq_range_id in output.small_heatmap.freq_range_id:
            freq_range_ids.append(freq_range_id)
        rates = []
        for rate in output.small_heatmap.rate:
            rates.append(rate)
        unsatisfied_ratios = []
        for unsatisfied_ratio in output.small_heatmap.unsatisfied_ratio:
            unsatisfied_ratios.append(unsatisfied_ratio)
        visualize['small_heatmap'] = {'num_rows': output.small_heatmap.num_rows, 'num_columns': output.small_heatmap.num_columns,'strength': strengths,
                                    'base_station_id': base_station_ids,'freq_range_id': freq_range_ids,'rate': rates,'unsatisfied_ratio': unsatisfied_ratios}
        persons = []
        ...
        visualize['statistics'] = {'num_agent':output.statistics.num_agent,'num_satisfied_agent':output.statistics.num_satisfied_agent,
                                'num_unsatisfied_agent':output.statistics.num_unsatisfied_agent,'num_outage_agent':output.statistics.num_outage_agent,
                                'num_outage_agent':output.statistics.num_outage_agent,'num_active_agent':output.statistics.num_active_agent,
                                'num_connect_agent':output.statistics.num_connect_agent,'demand_flow':output.statistics.demand_flow,
                                'actual_flow':output.statistics.actual_flow,'num_base_station':output.statistics.num_base_station,
                                'num_high_load_base_station':output.statistics.num_high_load_base_station,'num_low_load_base_station':output.statistics.num_low_load_base_station,
                                'num_ok_load_base_station':output.statistics.num_ok_load_base_station,'power_consumption':output.statistics.power_consumption}
